...
namespace control {
	...
	void PID::SetParameters(double dt,double p_gain, double i_gain, double d_gain, double clip_value) {
		dt_ = dt;
		p_gain_ = p_gain;
		i_gain_ = i_gain;
		d_gain_ = d_gain;
	...
	Eigen::Vector3d AngularRateController::KroneckerProduct(Eigen::Quaterniond q, Eigen::Quaterniond q_ref){
	...
	*/
	...
	double AngularRateController::Saturator(double input, double upper_limit, double lower_limit) {
