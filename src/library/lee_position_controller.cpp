...
namespace control {
	...
	*/
	...
	/*
		Saturator function
	*/
	double LeePositionController::Saturator(double input, double upper_limit,
  		double lower_limit) {
	...
	Eigen::Vector4d LeePositionController::CalculateRotorVelocities(
		...
		Eigen::Vector3d acceleration = k_x_*position_error + k_v_*velocity_error 
			- gravity_*e_3 - desire_trajectory.acceleration_W;
		...
		Eigen::Vector3d attidute_error = 0.5 *	SkewMatrix2Vector(R_c.transpose()*R 
			- R.transpose()*R_c);
		...
		if (logs) {
			...
			debug_file << "debug cs: angular velocity error roll: " << 
				angular_velocity_error.x() << "\n";
			debug_file << "debug cs: angular velocity error pitch: " << 
				angular_velocity_error.y() << "\n";
			debug_file << "debug cs: angular velocity error yaw: " << 
				angular_velocity_error.z() << "\n";
