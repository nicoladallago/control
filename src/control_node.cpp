...
ControlNode::ControlNode(ros::NodeHandle& nh) {
	...
	speed_msg_pub_ = nh.advertise<ros_kflytelemetry::DirectReference>("/kfly_1/DirectReference", 5);
	...
	kfly_sub_ = nh.subscribe("/kfly_1/IMUData", 10, &ControlNode::KflyCallBack, this, ros::TransportHints().tcpNoDelay());

	ros::ServiceClient client = nh.serviceClient<ros_kflytelemetry::ManageSubscription>("/kfly_1/ManageSubscription");
...
/*vicon_stream_sub = nh.subscribe("/vicon/darotor/darotor", 10, &ControlNode::ViconCallback, this, ros::TransportHints().tcpNoDelay()); */
...
Eigen::Vector3d ControlNode::ViconSpeed(Eigen::Vector3d current_pos,double sample_time, double lp_gain) {
...
Eigen::Vector3d ControlNode::ViconAngularVelocity(Eigen::Quaterniond current_orientation, double sample_time, double lp_gain) {

	// extract roll, pitch yaw
	Eigen::Vector3d orientation = lee_position_controller_.Quaternion2Euler(current_orientation);
	Eigen::Vector3d angular_velocity = (orientation - old_orientation) / sample_time;

	// low pass filtering
	angular_velocity_filtered_ = (1 - lp_gain)*angular_velocity + lp_gain*angular_velocity_filtered_;
	old_orientation = orientation;

	return angular_velocity_filtered_;
}

Eigen::Vector3d ControlNode::ViconAngularVelocityQuaternion(Eigen::Quaterniond current_orientation, double sample_time, double lp_gain) {
	...
	// low pass filtering
	angular_velocity_filtered_quat_ = (1 - lp_gain)*angular_velocity + lp_gain*angular_velocity_filtered_quat_;
	...
	while(ros::ok()) {
		...
		control_node.lee_position_controller_.ResetParameters(
			...
			*/
			...
			control_node.lee_position_controller_.Quaternion2Euler(control_node.orientation_vicon_);
			// adjust the yaw
			...
			Eigen::Quaterniond orientation_W = control_node.lee_position_controller_.Euler2Quaternion(euler_vicon);
			...
			*/
			...
			if ((control_node.position_vicon_ != Eigen::Vector3d::Zero()) && initial_calibration) {
				...
				velocity_W = control_node.ViconSpeed(position_W, sample_time, 0.7);

				angular_velocity_B_quat = orientation_W.toRotationMatrix() * control_node.ViconAngularVelocityQuaternion(orientation_W, sample_time, 0.7);
			...
			*/
			if (((std::abs(euler_vicon.x()) > 3.14/4) || (std::abs(euler_vicon.y()) > 3.14/4)) && (initial_calibration)) {
			...
			if (((std::abs(position_ref.x() - position_W.x()) > 0.2) || (std::abs(position_ref.y() - position_W.y()) > 0.2)  || (std::abs(position_ref.z() - position_W.z()) > 0.2)) && (initial_calibration)){
			...
			if (initial_calibration){
				...
				*/
				...
				Compute motor speed
				*/
				Eigen::Vector4d rotor_speed = control_node.lee_position_controller_.CalculateRotorVelocities(orientation_W, position_W, velocity_W, angular_rate_kfly, trajectory_ref, control_node.debug_file, logs_control_side);
				// saturate for safety
			...
			*/
			if (logs && initial_calibration) {
				...
				control_node.debug_file << "debug vicon rate: " << control_node.vicon_current_rate << "\n";
